...
import type { AreaChartData, AreaChartProps } from "./schemas.js";

const runtimeProps =
  defineProps<WidgetRuntimeProps<AreaChartData, AreaChartProps>>();
...
const option = computed(() => {
  ...
  const buildSeries = (name: string, values: Array<number | null>) => ({
    name,
    type: "line",
    smooth: runtimeProps.props.smooth ?? true,
    showSymbol: false,
  ...
  const series = seriesField
    ...
    : [
        buildSeries(
          runtimeProps.title ?? runtimeProps.t("chart.area.series.default"),
