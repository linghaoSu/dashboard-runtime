...
export async function loadExternalLanguage(lang: string) {
  ...
  const { module: bizModule } = await importWithLocaleFallback(
    lang,
    (l) => import(
      /* webpackChunkName: "biz-lang-[request]" */
      `@dao-style/biz/dist/locales/${l}/index.js`
    ),
    { fallback: 'en-US' },
  );
  const daoStyleBiz = (bizModule as { default: unknown })?.default;
